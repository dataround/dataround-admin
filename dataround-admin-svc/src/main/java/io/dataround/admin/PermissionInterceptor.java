...
public class PermissionInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!requestURI.startsWith("/dataadmin/api/")) {
            return true;
        }

        // Get current user from request attribute (set by AuthInterceptor)
        UserInfo currentUser = (UserInfo) request.getAttribute(Constants.CURRENT_USER);
        if (currentUser == null) {
        ...
        String apiKey = buildApiKey(requestURI.replace("/dataadmin", ""), method);
